...
class PageComponent {
  ...
  static function alert_danger(string $error_message): string {
    return <<<HTML
      <div class="alert alert-danger row align-items-center py-3 m-0 mb-2" role="alert">
        <svg class="col-auto" height="32" width="32" viewBox="0 0 16 16" fill="#ff7984" class="h-100 mt-2 col-2">
          <path
  ...
  static function home_button(): string {
    return <<<HTML
      <a class="home-button btn btn-outline-light m-4" href="/">Home</a>
    HTML;
  }

  static function page_footer(): string {
    return <<<HTML
      <footer class="bg-dark text-white text-center py-2">© 2024 BugTrackr</footer>
    HTML;
  }
  ...
  static function home_background(bool $logged_in): string {
    return
      $logged_in
      ? ''
      : '<img class="bg-image fixed-top z-n1 w-100 h-auto" src="' . Helper::get_resource_path('public/assets/generic_code_bg.png') . '">';
  }
